...
static bool
blob_bs_within_blob(struct spdk_bs_dev *dev, uint64_t lba, uint64_t lba_count)
{
	struct spdk_blob_bs_dev *b = (struct spdk_blob_bs_dev *)dev;
	struct spdk_blob *blob = b->blob;
	uint64_t	page;
	...
	* 	- no backing bdev (normal thick blob), or
	* 	- zeroes backing bdev (thin provisioned blob), or
	* 	- blob backing bdev (e.g snapshot)
	* In case of blob backing bdev, it may be possible that backing bdev has lesser
	* number of clusters than the child lvol blob because lvol blob has been
	* expanded after taking snapshot. In such a case, page will be outside the
	* cluster page range of the backing dev. For zeroes backing bdev,
	* this function should always return true. There could be scenario where some
	* other blob up the back devices chain has the cluster within_bounds if the main
	* (lvol)blob perhaps undergoes snapshot->shrink->snapshot->expand cycle, but we
	* shouldn't bother going up and refer that cluster.
	*/
	if (page >= blob->active.num_clusters * pages_per_cluster) {
		return false;
	}
...
bs_create_blob_bs_dev(struct spdk_blob *blob)
{
	...
	b->bs_dev.within_blob = blob_bs_within_blob;
